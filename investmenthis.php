<?php
  ...
  $stmt0 = $con->query("select * from u965831441_selfh.inout where user_id = '$id' and kind = 'investment'");
...
<p id="btn" class="add_bn"><img src="image/add_bn.png"></p>
    ...
    <div class="saving">
    <p class="savetext">INVESTMENT HISTORY</p>
    <p class="savemoney">&yen;<?= number_format($investment); ?>-</p>
  ...
  <?php
    while($row = $stmt0 -> fetch(PDO::FETCH_ASSOC)) {
      ...
      $toushi=number_format($row["outgo"]);
      echo '<tr>';
      echo '<td class="ttop">' .$day. '</td>';
      echo '<td class="midle">&yen;' .$toushi.'-</td>';
